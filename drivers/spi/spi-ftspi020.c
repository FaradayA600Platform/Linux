...
// Copyright (C) 2021-2025 Faraday Technology Corporation.
...
#define FTSPI020_REG_XIPFO              0x34
#define FTSPI020_REG_XIPAMB             0x38
#define FTSPI020_REG_XIPAOL             0x3c
...
#else
#define FTSPI020_CR2_DMA_RX_THOD        GENMASK(13, 12)
#define FTSPI020_CR2_DMA_TX_THOD        GENMASK(9, 8)
#endif
...
#ifdef CONFIG_SPI_FTSPI020_USE_DAMR
	struct resource *damr;
 ...
 */
static int ftspi020_wait_complete(struct ftspi020_priv *ctrl, signed long timeout)
{
	...
	do {
		isr = readl(ctrl->io_base + FTSPI020_REG_ISR);
		writel(isr, ctrl->io_base + FTSPI020_REG_ISR);
...
static int ftspi020_do_data_dma(struct ftspi020_priv *ctrl, const struct spi_mem_op *op)
{
	...
	if (op->data.dir == SPI_MEM_DATA_IN) {
		...
		if (op->data.nbytes % ctrl->dma_rxfifo_thod) {
			...
			dma_cfg.src_maxburst = ctrl->dma_rxfifo_thod / 4;
		}
	} else {
		...
		dma_cfg.src_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
		dma_cfg.dst_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
		if (op->data.nbytes % ctrl->dma_txfifo_thod)
			dma_cfg.src_maxburst = 1;
		else
			dma_cfg.src_maxburst = ctrl->dma_txfifo_thod;
	}

	if (dmaengine_slave_config(ctrl->dma_ch, &dma_cfg))
		dev_err(ctrl->dev, "dma slave config failed\n");
	...
	{
		dev_err(ctrl->dev, "wait dma complete timeout\n");
		err = -ETIMEDOUT;
	}
...
static int ftspi020_do_data_pio(struct ftspi020_priv *ctrl, const struct spi_mem_op *op)
{
	...
	while (len > 0) {
		...
		ret = readl_relaxed_poll_timeout_atomic(ctrl->io_base + FTSPI020_REG_STS,
							val, (val & fifo_ready_mask), 0,
							500000);
		if (ret) {
...
static int ftspi020_firecmd(struct ftspi020_priv *ctrl, u32 cmd3,
...
#if 0
{
	if ((op->cmd.opcode == 0x20) || (op->cmd.opcode == 0xD8)) {
		dev_info(ctrl->dev, "cmd0: %08x, cmd1: %08x, cmd2: %08x, cmd3: %08x\n",
			ioread32(ctrl->io_base + FTSPI020_REG_CMD0),
			ioread32(ctrl->io_base + FTSPI020_REG_CMD1),
			ioread32(ctrl->io_base + FTSPI020_REG_CMD2), cmd);
		...
		if (op->data.nbytes) {
			print_hex_dump(KERN_INFO, "data: ", DUMP_PREFIX_OFFSET, 16, 1,
				       op->data.buf.in, (op->data.nbytes > 64) ?
				       64 : op->data.nbytes, false);
		}
	}
}
#endif
...
#if 0
	dev_dbg(ctrl->dev, "damr - xipcmd: %08x, addr_offset: %px, nbytes: %d\n",
		ioread32(ctrl->io_base + FTSPI020_REG_XIPCMD),
		ctrl->damr_base + op->addr.val, op->data.nbytes);
...
static int ftspi020_setup(struct spi_device *spi)
{
	...
	dev_dbg(ctrl->dev, "clk %d, hz %d, CR=%08x\n",
		clk_rate, spi->max_speed_hz,
...
#ifdef CONFIG_SPI_FTSPI020_USE_DAMR
	if (ctrl->damr) {
		iowrite32(ctrl->damr->start, ctrl->io_base + FTSPI020_REG_XIPAMB);
		iowrite32(ctrl->damr_size, ctrl->io_base + FTSPI020_REG_XIPAOL);
	}
#endif

#ifdef CONFIG_SPI_FTSPI020_USE_DMA
	ctrl->dma_ch = dma_request_slave_channel(dev, "tx-rx");
	if (IS_ERR(ctrl->dma_ch)) {
		dev_err(dev, "dma request channel failed\n");
		ret = PTR_ERR(ctrl->dma_ch);
		ctrl->dma_ch = NULL;
	} else {
...
#ifdef CONFIG_SPI_FTSPI020_V1_7_0
	ctrl->dma_rxfifo_thod = FIELD_GET(FTSPI020_CR2_DMA_RX_THOD, readl(ctrl->io_base + FTSPI020_REG_CR2)) == 0 ?
		4 : (1 << FIELD_GET(FTSPI020_CR2_DMA_RX_THOD, readl(ctrl->io_base+ FTSPI020_REG_CR2))) << 3;
	ctrl->dma_txfifo_thod = FIELD_GET(FTSPI020_CR2_DMA_TX_THOD, readl(ctrl->io_base + FTSPI020_REG_CR2)) == 0 ?
		4 : (1 << FIELD_GET(FTSPI020_CR2_DMA_TX_THOD, readl(ctrl->io_base + FTSPI020_REG_CR2))) << 3;
#else
	ctrl->dma_rxfifo_thod = (FIELD_GET(FTSPI020_CR2_DMA_RX_THOD, readl(ctrl->io_base + FTSPI020_REG_CR2)) + 1) * 
		(FIELD_GET(FTSPI020_FEATURE_RXFIFO_DEPTH, readl(ctrl->io_base + FTSPI020_REG_FEATURE)));
	ctrl->dma_txfifo_thod = (FIELD_GET(FTSPI020_CR2_DMA_TX_THOD, readl(ctrl->io_base + FTSPI020_REG_CR2)) + 1) * 
		(FIELD_GET(FTSPI020_FEATURE_TXFIFO_DEPTH, readl(ctrl->io_base + FTSPI020_REG_FEATURE)));
#endif
#endif
...
static int ftspi020_probe(struct platform_device *pdev)
{
	...
	ret = ftspi020_default_setup(ctrl);
	if (ret) {
