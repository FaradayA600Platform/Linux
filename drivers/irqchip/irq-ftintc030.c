...
static void __init ftintc030_dist_init(void)
{
	...
	/* setup SPI target */
#ifdef CONFIG_FARADAY_FTINTC030_SPI_TARGET_INVERSE
	for (irq = 0; irq < nr_spi; irq += 32)
			   ...
			   unsigned int cpu_match_id, unsigned int cpu_match_mask)
{
	...
	writel(cpu_match_mask, base + FTINTC030_OFFSET_CPU_MATCH_MASK(0));
...
{
	void __iomem *base;
	unsigned int parent_irq, match_id, match_mask;
	const __be32 *tmp;
	...
	tmp = of_get_property(ftintc030_np, "cpu-match-mask", NULL);
	if (tmp == NULL)
		match_mask = 0x000;
	else
		match_mask = be32_to_cpu(*tmp);

	ftintc030_init(base, -1, match_id, match_mask);
