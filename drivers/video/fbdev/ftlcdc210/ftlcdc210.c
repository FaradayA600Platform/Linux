 ...
 * Driver for Faraday FTLCDC210 LCD Controller 
 *
 ...
 */
//#define CONFIG_FB_FTLCDC210_AUO_A036QN01_CPLD
//#define CONFIG_FB_FTLCDC210_TPO_TD070WGEC2
//#define CONFIG_FB_FTLCDC210_VESA_800x600
//#define CONFIG_FB_FTLCDC210_VESA_1024x768

#define CONFIG_FTLCDC210_NR_FB   4//5
...
#ifdef CONFIG_FB_FTLCDC210_TPO_TD070WGEC2
static struct fb_var_screeninfo ftlcdc210_default_var  = {
   .xres             = 800,
   .yres             = 480,
   .xres_virtual     = 800,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_640x480
static struct fb_var_screeninfo ftlcdc210_default_var  = {
   .xres             = 640,
   .yres             = 480,
   .xres_virtual     = 640,
   .yres_virtual     = 480,
   .bits_per_pixel   = 16,
   .pixclock         = 39722,       // 60hz  ==> by calculation
   .left_margin      = 48,          // horizontal back
   .right_margin     = 16,          // front
   .upper_margin     = 33,          // vertical back
   .lower_margin     = 10,          // vertical front
   .hsync_len        = 96,
   .vsync_len        = 2,
   .vmode            = FB_VMODE_NONINTERLACED,
   .sync             = 0,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_800x600
static struct fb_var_screeninfo ftlcdc210_default_var  = {
   .xres             = 800,
   .yres             = 600,
   .xres_virtual     = 800,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1024x768
static struct fb_var_screeninfo ftlcdc210_default_var  = {
   .xres             = 1024,
   .yres             = 768,
   .xres_virtual     = 1024,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1280x720
static struct fb_var_screeninfo ftlcdc210_default_var = {
   .xres             = 1280,
   .yres             = 720,
   .xres_virtual     = 1280,
   .yres_virtual     = 720,
   .bits_per_pixel   = 16,
   .pixclock         = 11982,       // 60hz  ==> 依公式算
   .left_margin      = 200,         // horizontal back
   .right_margin     = 64,          // front
   .upper_margin     = 24,          // vertical back
   .lower_margin     = 1,           // vertical front
   .hsync_len        = 136,
   .vsync_len        = 3,
   .vmode            = FB_VMODE_NONINTERLACED,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1280x800
static struct fb_var_screeninfo ftlcdc210_default_var = {
   .xres             = 1280,
   .yres             = 800,
   .xres_virtual     = 1280,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1280x1024
static struct fb_var_screeninfo ftlcdc210_default_var = {
   .xres             = 1280,
   .yres             = 1024,
   .xres_virtual     = 1280,
   .yres_virtual     = 1024,
   .bits_per_pixel   = 16,
   .pixclock         = 9259,        // 60hz  ==> 依公式算
   .left_margin      = 248,         // horizontal back
   .right_margin     = 48,          // front
   .upper_margin     = 38,          // vertical back
   .lower_margin     = 1,           // vertical front
   .hsync_len        = 112,
   .vsync_len        = 3,
   .vmode            = FB_VMODE_NONINTERLACED,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1368x768
static struct fb_var_screeninfo ftlcdc210_default_var = {
   .xres             = 1368,
   .yres             = 768,
   .xres_virtual     = 1368,
   .yres_virtual     = 768,
   .bits_per_pixel   = 16,
   .pixclock         = 11688,       // 60hz  ==> 依公式算
   .left_margin      = 216,         // horizontal back
   .right_margin     = 72,          // front
   .upper_margin     = 23,          // vertical back
   .lower_margin     = 1,           // vertical front
   .hsync_len        = 144,
   .vsync_len        = 3,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1440x900
static struct fb_var_screeninfo ftlcdc210_default_var = {
   .xres             = 1440,
   .yres             = 900,
   .xres_virtual     = 1440,
   .yres_virtual     = 900,
   .bits_per_pixel   = 16,
   .pixclock         = 9392,        // 60hz  ==> 依公式算
   .left_margin      = 232,         // horizontal back
   .right_margin     = 80,          // front
   .upper_margin     = 28,          // vertical back
   .lower_margin     = 1,           // vertical front
   .hsync_len        = 152,
   .vsync_len        = 3,
   .vmode            = FB_VMODE_NONINTERLACED,
...
#ifdef CONFIG_FB_FTLCDC210_VESA_1680x1050
static struct fb_var_screeninfo ftlcdc210_default_var = {
   .xres             = 1680,
   .yres             = 1050,
   .xres_virtual     = 1680,
   .yres_virtual     = 1050,
   .bits_per_pixel   = 16,
   .pixclock         = 6796,        // 60hz  ==> 依公式算
   .left_margin      = 288,         // horizontal back
   .right_margin     = 104,         // front
   .upper_margin     = 33,          // vertical back
   .lower_margin     = 1,           // vertical front
   .hsync_len        = 184,
   .vsync_len        = 3,
...
#ifdef CONFIG_FB_FTLCDC210_CEA_1920x1080
static struct fb_var_screeninfo ftlcdc210_default_var = {
	.xres           = 1920,
	.yres           = 1080,
	.xres_virtual   = 1920,
	.yres_virtual   = 1080,
	.bits_per_pixel = 16,
	.pixclock       = 6734,         // 60hz	==> 依公式算
	.left_margin    = 148,          // horizontal back
	.right_margin   = 88,           // front
	.upper_margin   = 36,           // vertical back
	.lower_margin   = 4,            // vertical front
	.hsync_len      = 44,
	.vsync_len      = 5,
	.vmode          = FB_VMODE_NONINTERLACED,
	.sync           = 0,
      ...
      unsigned int value)
{
   ...
   //printk("[%s]  [FRAME BASE0] = %08x\n",__func__, value);
   //printk("[%s]  [FRAME BASE1] = %08x\n",__func__, (value+pftlcdc210->u_offset));
   //printk("[%s]  [FRAME BASE2] = %08x\n",__func__, (value+pftlcdc210->v_offset));
#else
   dev_dbg(pftlcdc210->dev, "  [FRAME BASE] = %08x\n", value);
      ...
      struct fb_var_screeninfo *var)
{
   ...
   #else
      screen_base = dma_alloc_wc(dev, smem_len,   (dma_addr_t *)&smem_start, GFP_KERNEL | GFP_DMA);
      //printk("[%s] Enter!! PATH dma_alloc_wc !! screen_base = 0x%x smem_start=0x%x\n",__func__,screen_base,smem_start);
   ...
   if (info->screen_base) {
      ...
      #else
         dma_free_wc(dev, info->fix.smem_len, info->screen_base, (dma_addr_t )info->fix.smem_start);
      #endif
   }
 ...
 * interrupt handler
 *****************************************************************************/
#ifdef CONFIG_FB_FTLCDC210_USE_INT
...
#endif /* CONFIG_FB_FTLCDC210_USE_INT */
...
static int FTLCDC210_fb0_set_par(struct fb_info *info)
{
   ...
   pftlcdc210fb->FTLCDC210->u_offset = info->var.xres * info->var.yres;
   pftlcdc210fb->FTLCDC210->v_offset = info->var.xres * info->var.yres * 5 / 4;
   //
#endif
...
static int faradayfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
   ...
   //printk("[%s] Enter!! cmd = 0x%x arg = 0x%x\n",__func__,cmd,arg);
   ...
   switch (cmd)
   {
      ...
      case FBTESTIMGBLT:
         ...
         //DMA PATH
         src_buf = dma_alloc_wc(info->device, (p->size*p->height),   (dma_addr_t *)&dma_src_buf, GFP_KERNEL | GFP_DMA);
         if (!src_buf)
         {
            ret = -EFAULT;
            goto error_imgblt;
...
error_imgblt:
         ...
         //DMA PATH
         dma_free_wc(info->device, (p->size*p->height), src_buf, dma_src_buf);
...
static int ftlcdc210_probe(struct platform_device *pdev)
{
   ...
   if (pdev->dev.of_node) {
      ...
      clk = devm_clk_get(&pdev->dev, "scaclk");
      if (!IS_ERR(clk)) {
         clk_prepare_enable(clk);
         pftlcdc210->scaclk = clk;
      }

      clk = devm_clk_get(&pdev->dev, "lcclk");
      if (IS_ERR(clk)) {
         dev_err(dev, "Failed to get clock(lcclk)\n");
         ret = PTR_ERR(clk);
         goto err_clk_get;
      }
      clk_prepare_enable(clk);
      pftlcdc210->clk = clk;
      pftlcdc210->clk_value_khz = clk_get_rate(clk)/1000;//0x30570;
   }
   else
   {
   ...
   rstc = devm_reset_control_get(&pdev->dev, "rstn_lcclk");
   if (!IS_ERR_OR_NULL(rstc))
      reset_control_deassert(rstc);

   rstc = devm_reset_control_get(&pdev->dev, "rstn_scaclk");
   if (!IS_ERR_OR_NULL(rstc))
      reset_control_deassert(rstc);

   //Map io memory
   pftlcdc210->base = devm_ioremap_resource(&pdev->dev, res);
   if (!pftlcdc210->base) {
   ...
   ret = devm_request_irq(&pdev->dev, irq_be, ftlcdc210_interrupt, IRQF_SHARED, pdev->name, pftlcdc210);
   if (ret < 0) {
      dev_err(dev, "Failed to request irq %d\n", irq_be);
      goto err_req_irq_be;
   }

   ret = devm_request_irq(&pdev->dev, irq_ur, ftlcdc210_interrupt, IRQF_SHARED, pdev->name, pftlcdc210);
   if (ret < 0) {
      dev_err(dev, "Failed to request irq %d\n", irq_ur);
      goto err_req_irq_ur;
   }

   ret = devm_request_irq(&pdev->dev, irq_nb, ftlcdc210_interrupt, IRQF_SHARED, pdev->name, pftlcdc210);
   if (ret < 0) {
      dev_err(dev, "Failed to request irq %d\n", irq_nb);
      goto err_req_irq_nb;
   }

   ret = devm_request_irq(&pdev->dev, irq_vs, ftlcdc210_interrupt, IRQF_SHARED, pdev->name, pftlcdc210);
   if (ret < 0) {
      dev_err(dev, "Failed to request irq %d\n", irq_vs);
      goto err_req_irq_vs;
   }
