...
/* fotg210_handshake - spin reading hc until handshake completes or fails
 ...
 */
static int fotg210_handshake(struct fotg210_hcd *fotg210, void __iomem *ptr,
...
static int fotg210_halt(struct fotg210_hcd *fotg210)
{
	...
	return fotg210_handshake(fotg210, &fotg210->regs->status,
...
static int fotg210_reset(struct fotg210_hcd *fotg210)
{
	...
	retval = fotg210_handshake(fotg210, &fotg210->regs->command,
...
static void fotg210_quiesce(struct fotg210_hcd *fotg210)
{
	...
	fotg210_handshake(fotg210, &fotg210->regs->status, STS_ASS | STS_PSS,
			temp, 16 * 125);
...
static int fotg210_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	if((temp & PORT_RESET) !=0) {
		if((temp & mask) != 0) {
			...
			fotg210->unplug_do_hc_rst = 1; //lichun@add
		}
	}
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetPortStatus:
		...
		if (temp & (PORT_CONNECT | PORT_CSC | PORT_PE | PORT_PEC))
			/*fotg210->unplug_do_hc_rst = 1*/; //disable it because HW >= v1.28 fix
		else {
			if (fotg210->unplug_do_hc_rst) {
				fotg210_set_hc_reset(fotg210);
				fotg210->unplug_do_hc_rst = 0;
			}
		}
		//~lichun@2019
		...
		if (temp & PORT_RESUME) {
			...
			/* resume completed? */
				...
				retval = fotg210_handshake(fotg210, status_reg,
		...
		if (temp & PORT_RESET) { //fix jiffies overflow after boot 5 min
		   if ((fotg210->reset_done[wIndex]) && time_after_eq(jiffies,
				fotg210->reset_done[wIndex])) {
...
retry_xacterr:
		...
		qh->xacterrs = 0;
#if 0
		//lichun@patch, CX transfer: PING bit set shall be with OUT PID code (HW < v1.31)
		if ((usb_pipetype(urb->pipe) == PIPE_CONTROL) &&
		    (QTD_PID(token) != 0) && (token & QTD_STS_PING)) {
			printk("... WARN: CX status stage qtd wrong!\n");
			token &= ~QTD_STS_PING;
			qtd->hw_token = cpu_to_hc32(fotg210, token);
			wmb();
			hw->hw_token = cpu_to_hc32(fotg210, token);
		}
#endif
	}
...
/* 20150829 BC: delay hc_reset, let AP knows usb disconnect, will call fotg210_set_hc_reset() */
		...
		if (fotg210->unplug_do_hc_rst && (pstatus & PORT_CSC)) {
...
static void fotg210_init(struct fotg210_hcd *fotg210)
{
	...
	iowrite32(GMIR_MDEV_INT | GMIR_MOTG_INT | GMIR_INT_POLARITY,
			&fotg210->regs->gmir);
	#else
	iowrite32(GMIR_MDEV_INT | GMIR_MOTG_INT,
			&fotg210->regs->gmir);
	#endif
