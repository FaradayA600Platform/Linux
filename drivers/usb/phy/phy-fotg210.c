 ...
 *
 * Copyright (C) 2016-2021 Faraday Technology Corp.
 *
...
run:
	...
	if (old_state != fotg210->phy.otg->state) {
		...
		case OTG_STATE_B_IDLE:
			...
			iowrite32(GMIR_MHC_INT | GMIR_INT_POLARITY, fotg210->regs + FOTG210_GMIR);
			#else
		...
		case OTG_STATE_A_IDLE:
			...
			#ifdef CONFIG_INT_POLARITY_HIGH
			iowrite32(GMIR_MDEV_INT | GMIR_INT_POLARITY, fotg210->regs + FOTG210_GMIR);
			#else
...
static void fotg210_init(struct fotg210_otg *fotg210)
{
	...
	fotg210->irq_en = INT_ROLECHG; //INT_IDCHG;
