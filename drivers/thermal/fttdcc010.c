                              ...
                              int *temp)
{
	...
	val = readl(&priv->regs->data[thermal->id]) & 0x3FF;
...
static int fttdcc010_change_mode(struct thermal_zone_device *thermal,
                            ...
                            int trip, enum thermal_trip_type type)
{
	...
	switch (type) {
		case THERMAL_TRIP_CRITICAL:
			...
			zone = list_first_entry(&priv->head, struct fttdcc010_thermal_zone, list);
			thermal_zone_device_disable(zone->thermal);
			break;
		default:
			break;
	}
...
static struct thermal_zone_device_ops ops = {
	...
	.change_mode   = fttdcc010_change_mode,
...
static int fttdcc010_probe(struct platform_device *pdev)
{
	...
	thermal_zone_device_enable(zone->thermal);
...
{
	struct fttdcc010_thermal_priv *priv = platform_get_drvdata(pdev);
	struct fttdcc010_thermal_zone *zone;
