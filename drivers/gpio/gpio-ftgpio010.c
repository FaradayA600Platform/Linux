...
static int ftgpio010_probe(struct platform_device *pdev)
{
	...
	/* ungated gpio apb clock */
	clk = devm_clk_get(&pdev->dev, "pclk");
	if (!IS_ERR(clk)) {
		clk_prepare_enable(clk);
	}

	/* deasserted gpio reset */
	...
	girq = &ftgpio010->gpio_chip.irq;
	girq->chip = &ftgpio010_irqchip;
	girq->parent_handler = ftgpio010_irq_handler;
	girq->num_parents = 1;
	girq->parents = devm_kcalloc(&pdev->dev, 1, sizeof(*girq->parents),
	                             GFP_KERNEL);
	if (!girq->parents) {
		dev_err(&pdev->dev, "Failed to allocate parenet memory.\n");
		ret = -ENOMEM;
		goto err_chip;
	}
	...
	ret = devm_gpiochip_add_data(&pdev->dev, &ftgpio010->gpio_chip, ftgpio010);
	if (ret) {
		dev_err(&pdev->dev, "devm_gpiochip_add_data() failed, %d\n", ret);
		goto err_irqchip;
	}
