...
static irqreturn_t ftpwmtmr010_interrupt(int irq, void *dev_id)
{
	struct ftpwmtmr010_pwm_chip *ftc_pwm_chip = dev_id;
	struct ftpwmtmr010_regs *regs = ftc_pwm_chip->mmio;
	unsigned int val;
...
static int ftpwmtmr010_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	val = readl(&regs->timer[pwm->hwpwm].ctrl);
	val |= CTRL_AUTORELOAD | CTRL_UPDATE | CTRL_START |CTRL_PWMEN | CTRL_INTEN;
	writel(val, &regs->timer[pwm->hwpwm].ctrl);
...
static int ftpwmtmr010_pwm_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
	pwm->clk = devm_clk_get(&pdev->dev, "pclk");
#else
	...
	if (ret < 0) {
		dev_err(pwm->chip.dev, "failed to get the irq\n");
		goto err_req_irq;
	}
	irq = ret;

	/* Register irq */
	ret = devm_request_irq(&pdev->dev,irq, ftpwmtmr010_interrupt, 0, pdev->name, pwm);
	if (ret < 0) {
		dev_err(pwm->chip.dev, "Failed to request irq %d\n", irq);
		goto err_req_irq;
	}
	...
	dev_info(&pdev->dev, "pwm probe successful\n");
