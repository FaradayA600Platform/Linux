...
static void __iomem *fttmr010_base = 0;
static unsigned int clk_reload;
static bool fttmr010_is_level_trigger;
...
static void fttmr010_mask_irq(unsigned int id)
{
	unsigned int mask;
...
static irqreturn_t fttmr010_clockevent_interrupt(int irq, void *dev_id)
{
	...
	if (!(state & fttmr010_irq_mask[EVTTMR]))
		return IRQ_NONE;
	/*
	 * If IRQ is level trigger mode, FTTMR010 driver need to clear FTTMR010 IRQ source.
	 * If IRQ is edge trigger mode, interrupt controller will clear the IRQ status
	 * automatically.
	 */
	if(fttmr010_is_level_trigger)
...
int __init fttmr010_init(void __iomem *base, int irq, unsigned long clk_freq)
{
	...
	ret = request_irq(irq, fttmr010_clockevent_interrupt,
			IRQF_TIMER | IRQF_IRQPOLL, "fttmr010", &fttmr010_clkevt);
	if (ret) {
                pr_err("Failed to register timer IRQ\n");
                return ret;
	}

	/* Check the irq trigger mode */
	desc = irq_to_desc(irq);
	fttmr010_is_level_trigger = irqd_is_level_type(&desc->irq_data);

	/* setup struct clock_event_device */
#if defined(CONFIG_SMP)
	fttmr010_clkevt.cpumask	= cpu_possible_mask;
#else
	fttmr010_clkevt.cpumask	= cpumask_of(0);
#endif
	fttmr010_clkevt.irq = irq;
...
#ifdef CONFIG_FARADAY_A500_ERRATUM_000001
...
TIMER_OF_DECLARE(fttmr010, "faraday,fttmr010", fttmr010_of_init);

//Fix A500 Timer Bug
#ifdef CONFIG_FARADAY_A500_ERRATUM_000001
unsigned long long fttmr010_get_tick(void)
{
	if (fttmr010_base == 0) {
		return 0;
	}
	else {
		return fttmr010_sched_clock_read();
	}
}
#endif
