...
#define DRIVER_VERSION  "05-Oct-2021"
...
static int ftadcc010_probe(struct platform_device *pdev)
{
	...
	indio_dev = iio_device_alloc(dev, sizeof(*adc));
	if (!indio_dev)
		return -ENOMEM;

	adc = iio_priv(indio_dev);

	/* ungated adc apb clock */
...
{
	struct ftadcc010 *adc = platform_get_drvdata(pdev);

	if(adc->rc)
		reset_control_assert(adc->rc);
	if (adc->mclk)
		clk_disable_unprepare(adc->mclk);
	if (adc->pclk)
...
{
	struct ftadcc010 *adc = dev_get_drvdata(dev);
