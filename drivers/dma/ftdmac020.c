...
static unsigned int ftdmac020_get_transfer_size(struct ftdmac020_chan *ftchan)
{
	void __iomem *base = ftchan->ftdmac020->base;
	int chan_id = ftchan->common.chan_id;
	unsigned int temp_reg;

	temp_reg = ioread32(base + FTDMAC020_OFFSET_CYC_CH(chan_id));

	return (temp_reg);
}

static unsigned int ftdmac020_get_bus_width(struct ftdmac020_chan *ftchan)
{
	void __iomem *base = ftchan->ftdmac020->base;
	int chan_id = ftchan->common.chan_id;
	unsigned int temp_reg;
	...
	temp_reg = ioread32(base + FTDMAC020_OFFSET_CCR_CH(chan_id));

	//get src_data_width
	temp_reg = temp_reg & 0x00003800;
	...
	switch (temp_reg)
	{
		case 0x0://8 bit = 1 Byte
			bus_width_size = 1;
			break;
		case 0x00000800:// 16bits = 2 Bytes
			bus_width_size = 2;
			break;
		case 0x00001000:// 32bits = 4 Bytes
			bus_width_size = 4;
			break;
		case 0x00001800:// 64bits = 8 Bytes
			bus_width_size = 8;
			break;
		default:
			bus_width_size = 4;
...
static void ftdmac020_start_chain(struct ftdmac020_desc *desc)
{
	...
	dev_dbg(chan2dev(chan), "\t[SRC %d] = %llx\n", chan_id, (unsigned long long)desc->src);
	iowrite32(desc->src, base + FTDMAC020_OFFSET_SRC_CH(chan_id));
	dev_dbg(chan2dev(chan), "\t[DST %d] = %llx\n", chan_id, (unsigned long long)desc->dst);
	iowrite32(desc->dst, base + FTDMAC020_OFFSET_DST_CH(chan_id));
	dev_dbg(chan2dev(chan), "\t[LLP %d] = %llx\n", chan_id, (unsigned long long)desc->next);
		...
		unsigned int src_shift, unsigned int dst_shift, int maxburst, int fixed_src, int fixed_dest)
{
	...
	dev_dbg(chan2dev(chan),
		"%s(src %llx, dest %llx, len %lx, src_shift %d, dst_shift %d, maxburst %d)\n",
		__func__, (unsigned long long)src, (unsigned long long)dest, (unsigned long)len, src_shift, dst_shift, maxburst);
		...
		unsigned int shift, int maxburst, int fixed_src, int fixed_dest)
{
	...
	dev_dbg(chan2dev(chan),
		"%s(src %llx, dest %llx, len %lx, shift %d, maxburst %d)\n",
		__func__, (unsigned long long)src, (unsigned long long)dest, (unsigned long)len, shift, maxburst);
			  ...
			  unsigned long flags)
{
	...
	dev_dbg(chan2dev(chan), "%s: buf_len = 0x%lx period_len = 0x%lx\n", __func__,
		(unsigned long)buf_len, (unsigned long)period_len);
		...
		struct dma_tx_state *txstate)
{
	...
	if (ftchan->cyclic)
	{
		data_byte = ftdmac020_get_bus_width(ftchan);
		//printk("[%s] data_byte = 0x%x \n",__func__,data_byte);
		transfer_size = ftdmac020_get_transfer_size(ftchan);
...
/**
 * ftdmac020_release - release dma device
 * @dev: target DMA device
 */
static void ftdmac020_release(struct dma_device *dev)
{
	//dev_dbg(chan2dev(chan), "%s\n", __func__);
}
...
static int ftdmac020_probe(struct platform_device *pdev)
{
	...
	dma->device_release              = ftdmac020_release;
	...
	dev_info(&pdev->dev, "DMA engine driver: tc irq %d, mapped at %px\n",
...
static int __exit ftdmac020_remove(struct platform_device *pdev)
{
	...
	/* gated dmac ahb clock */
	if (ftdmac020->clk)
		clk_disable_unprepare(ftdmac020->clk);

	/* asserted dmac reset */
	if (ftdmac020->rc)
		reset_control_assert(ftdmac020->rc);
