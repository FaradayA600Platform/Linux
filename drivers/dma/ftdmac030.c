...
struct ftdmac030 {
	...
	unsigned int arUserWidth;
	unsigned int awUserWidth;
...
static unsigned int ftdmac030_get_transfer_size(struct ftdmac030_chan *ftchan)
{
	void __iomem *base = ftchan->ftdmac030->base;
	int chan_id = ftchan->common.chan_id;
	unsigned int temp_reg;

	temp_reg = ioread32(base + FTDMAC030_OFFSET_CYC_CH(chan_id)); 

	return (temp_reg);
}

static unsigned int ftdmac030_get_bus_width(struct ftdmac030_chan *ftchan)
{
	void __iomem *base = ftchan->ftdmac030->base;
	int chan_id = ftchan->common.chan_id;
	unsigned int temp_reg;
	unsigned int bus_width_size;

	temp_reg = ioread32(base + FTDMAC030_OFFSET_CTRL_CH(chan_id)); 

	//get src_data_width
	temp_reg = temp_reg & 0x0E000000;

	switch (temp_reg)
	{
		case 0x0://8 bit = 1 Byte
			bus_width_size = 1;
			break;
		case 0x02000000:// 16bits = 2 Bytes
			bus_width_size = 2;
			break;
		case 0x04000000:// 32bits = 4 Bytes
			bus_width_size = 4;
			break;
		case 0x05000000:// 64bits = 8 Bytes
			bus_width_size = 8;
			break;
		case 0x08000000:// 128bits = 16 Bytes
			bus_width_size = 16;
			break;
		default:
			bus_width_size = 2;
	}
	return (bus_width_size);
}
		...
		struct ftdmac030_chan *ftchan, gfp_t gfp_flags)
{
	...
	//dev_dbg(chan2dev(chan), "[%s] Enter!! gfp_flags = 0x%x\n", __func__,gfp_flags);
	...
	//dev_dbg(chan2dev(chan), "[%s] Leave!! desc = %p\n", __func__,desc);
	return desc;
}
...
static void ftdmac030_start_chain(struct ftdmac030_desc *desc)
{
	...
	if (ftchan->ftdmac030->axi_AddrWidth > 32)
	{//support 48 bit
		dev_dbg(chan2dev(chan), "\t[ADR EXP %d] = %x\n", chan_id, desc->lld.addr_exp);
		iowrite32(desc->lld.addr_exp, base + FTDMAC030_OFFSET_ADR_EXP_CH(chan_id));
		dev_dbg(chan2dev(chan), "\t[LLP EXP %d] = %x\n", chan_id, desc->lld.llp_exp);
		iowrite32(desc->lld.llp_exp, base + FTDMAC030_OFFSET_LLP_EXP_CH(chan_id));
	}
}
		...
		unsigned int shift, int maxburst, int fixed_src, int fixed_dest)
{
	...
	size_t offset;
	unsigned int cycle;
	dma_addr_t llp_exp = 0;
	dma_addr_t high_src = 0;
	dma_addr_t high_dest = 0;
	dma_addr_t add_mask = 0;
	dma_addr_t temp = 0;

	dev_dbg(chan2dev(chan), "%s(src %llx, dest %llx, len %lx, shift %d)\n",
		__func__, (unsigned long long)src, (unsigned long long)dest, (unsigned long)len, shift);
	...
	for (offset = 0; offset < len; offset += cycle << shift) {
		...
		if (ftchan->ftdmac030->axi_AddrWidth == 48)
		{//support 48 bit
			add_mask = 0xffff;
			add_mask = add_mask<<16;
			add_mask = add_mask<<16;
			
			high_src = (src&add_mask)>>16;
			high_src = high_src>>16;
			
			high_dest = (dest&add_mask)>>16;
			high_dest = high_dest>>16;
			
			desc->lld.addr_exp = (high_dest<<16)|high_src;
			
			dev_dbg(chan2dev(chan), "%s(add_mask=0x%llx, high_src=0x%llx, high_dest=0x%llx)\n", __func__, (unsigned long long)add_mask, (unsigned long long)high_src, (unsigned long long)high_dest);
			dev_dbg(chan2dev(chan), "%s(desc->lld.addr_exp=0x%x)\n",__func__, desc->lld.addr_exp);
		}
		...
		if (!first) {
			...
			{//support 48 bit
				...
				temp = (dma_addr_t)(&desc->txd.phys);
				llp_exp = (temp&add_mask);
				llp_exp = llp_exp>>16;
				llp_exp = llp_exp>>16;
				
				prev->lld.llp_exp = (ftchan->ftdmac030->arUserWidth<<24)|(ftchan->ftdmac030->awUserWidth<<16)|(llp_exp);        
				dev_dbg(chan2dev(chan), "%s(prev->lld.llp_exp=0x%x, temp=0x%llx)\n",__func__, prev->lld.llp_exp, (unsigned long long)temp);
			}
		...
		unsigned int shift, int maxburst, int fixed_src, int fixed_dest)
{
	...
	dev_dbg(chan2dev(chan), "%s: src = 0x%llx, dest = 0x%llx, len = 0x%lx, shift = %d\n",
		__func__, (unsigned long long)src, (unsigned long long)dest, (unsigned long)len, shift);
	...
	for (offset = 0; offset < len; offset += cycle << shift) {
		...
		{//support 48 bit
			...
			high_src = (desc->lld.src&add_mask)>>16;
			high_src = high_src>>16;
			
			high_dest = (desc->lld.dst&add_mask)>>16;
			high_dest = high_dest>>16;
		...
		unsigned int shift, int maxburst)
{
	...
	dev_dbg(chan2dev(chan), "%s(dest %llx, len %lx, value %x, shift %d)\n",
		__func__, (unsigned long long)dest, (unsigned long)len, value, shift);
		...
		unsigned int shift, int maxburst, int fixed_src, int fixed_dest)
{
	...
	for (offset = 0; offset < len; offset += cycle ) 
	{
		...
		dev_dbg(chan2dev(chan), "[%s] offset=0x%lx len=0x%lx cycle=0x%x shift=0x%x\n",
			__func__,(unsigned long)offset,(unsigned long)len,cycle,shift);
		...
		int src_stride,int dst_stride)
{
	...
	dev_dbg(chan2dev(chan), "%s(src 0x%llx, dest 0x%llx, shift %d maxburst %d)\n",
		__func__, (unsigned long long)src, (unsigned long long)dest, shift,maxburst);
...
static void ftdmac030_tasklet(unsigned long data)
{
	...
	if (! list_empty(&ftchan->active_list))
	{
		if (ftchan->cyclic)
		{
			...
			if (ftchan->residue == 0) ftchan->residue = ftchan->buffer_len;
...
static irqreturn_t ftdmac030_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < CHANNEL_NR; i++) {
		...
		if (eas & FTDMAC030_EA_ERR_CH(i))
			dev_err(chan2dev(chan), "error happened!! eas = 0x%x tcs = 0x%x\n",eas,tcs);
		
		if (eas & FTDMAC030_EA_ABT_CH(i))
			dev_warn(chan2dev(chan), "transfer aborted!! eas = 0x%x\n",eas);
		...
		dma_addr_t src, size_t len, unsigned long flags)
{
	...
	dev_dbg(chan2dev(chan), "%s(src %llx, dest %llx, len %lx)\n",
		__func__, (unsigned long long)src, (unsigned long long)dest, (unsigned long)len);
	ftchan = container_of(chan, struct ftdmac030_chan, common);
	ftchan->cyclic = false;
		...
		size_t len, unsigned long flags)
{
	...
	dev_dbg(chan2dev(chan), "[%s] dest=0x%llx, len=0x%lx, value=0x%x\n",
		__func__, (unsigned long long)dest, (unsigned long)len, value);
	ftchan = container_of(chan, struct ftdmac030_chan, common);
	ftchan->cyclic = false;
	...
	dev_dbg(chan2dev(chan), "[%s] (align_dest=0x%llx, align_len=0x%lx, word_value=0x%x,end_dest_len=0x%lx)\n",
		__func__, (unsigned long long)align_dest, (unsigned long)align_len, word_value, (unsigned long)end_dest_len);
		...
		unsigned long flags)
{
	...
	dev_dbg(chan2dev(chan), "%s: buf_len = 0x%llx period_len = 0x%lx\n", __func__,
		(unsigned long long)buf_len,(unsigned long)period_len);
		...
		struct dma_interleaved_template *xt, unsigned long flags)
{
	...
	dev_dbg(chan2dev(chan), "[%s] src_start=0x%llx dst_start=0x%llx dir=0x%x\n", __func__,(unsigned long long)xt->src_start,(unsigned long long)xt->dst_start,xt->dir);
	dev_dbg(chan2dev(chan), "[%s] src_inc=0x%x dst_inc=0x%x src_sgl=0x%x\n", __func__,xt->src_inc,xt->dst_inc,xt->src_sgl);
	dev_dbg(chan2dev(chan), "[%s] dst_sgl=0x%x numf=0x%lx frame_size=0x%lx\n", __func__,xt->dst_sgl,(unsigned long)xt->numf,(unsigned long)xt->frame_size);
	dev_dbg(chan2dev(chan), "[%s] sgl[0].size=0x%lx sgl[0].icg=0x%lx\n", __func__,(unsigned long)xt->sgl[0].size,(unsigned long)xt->sgl[0].icg);
	dev_dbg(chan2dev(chan), "[%s] sgl[0].dst_icg=0x%lx sgl[0].src_icg=0x%lx\n", __func__,(unsigned long)xt->sgl[0].dst_icg,(unsigned long)xt->sgl[0].src_icg); 
		...
		struct dma_tx_state *txstate)
{
	...
	if (ftchan->cyclic) 
	{
		data_byte = ftdmac030_get_bus_width(ftchan);
		//printk("[%s] data_byte = 0x%x \n",__func__,data_byte);
		transfer_size = ftdmac030_get_transfer_size(ftchan);
		//printk("[%s] ftchan->residue = 0x%x transfer_size = 0x%x\n",__func__,ftchan->residue,transfer_size);
		residue = ftchan->residue - ftchan->period_len + transfer_size * data_byte;
		//printk("[%s] residue = 0x%x \n",__func__,residue);
...
/**
 * ftdmac030_release - release dma device
 * @dev: target DMA device
 */
static void ftdmac030_release(struct dma_device *dev)
{
	//dev_dbg(chan2dev(chan), "%s\n", __func__);
}
...
static int ftdmac030_probe(struct platform_device *pdev)
{
	...
	dma->device_release              = ftdmac030_release;
	...
	//Read feature register2
	temp_reg = ioread32(ftdmac030->base + FTDMAC030_OFFSET_FEATURE2);
	ftdmac030->arUserWidth = (temp_reg&0xf0000)>>16;
	ftdmac030->awUserWidth = (temp_reg&0xf00)>>8;
	ftdmac030->axi_AddrWidth = (temp_reg&0x3f);

	//dev_dbg(&pdev->dev, "DMAC030 Feature register2 = 0x%x!!\n",temp_reg);
	//dev_dbg(&pdev->dev, "DMAC030 ftdmac030->arUserWidth = 0x%x!!\n",ftdmac030->arUserWidth);
	//dev_dbg(&pdev->dev, "DMAC030 ftdmac030->awUserWidth = 0x%x!!\n",ftdmac030->awUserWidth);
	//dev_dbg(&pdev->dev, "DMAC030 ftdmac030->axi_AddrWidth = 0x%x!!\n",ftdmac030->axi_AddrWidth);

	if (ftdmac030->axi_AddrWidth == 48)
	{
		//Set device dma mask to 48
		err = dma_set_mask_and_coherent(&pdev->dev, DMA_BIT_MASK(48));
		if (err) {
			dev_err(&pdev->dev, "48-bit DMA addressing not available\n");
			goto err_register;
		}
	}
	else
	{
		//Set device dma mask to 32
	...
	dev_info(&pdev->dev,
		"DMA engine driver: irq %d, mapped at %px\n",
		irq, ftdmac030->base);
