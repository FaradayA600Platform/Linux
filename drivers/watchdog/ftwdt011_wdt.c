...
{
	struct ftwdt011 *ftwdt011 = watchdog_get_drvdata(wdd);
	unsigned long clkrate = ftwdt011->extclk ? clk_get_rate(ftwdt011->extclk) : clk_get_rate(ftwdt011->clk);
	unsigned int counter;
	unsigned long flags;
...
static int ftwdt011_probe(struct platform_device *pdev)
{
	...
	ftwdt011->wdd.timeout = FTWDT011_DEFAULT_TIMEOUT;
	...
	watchdog_init_timeout(&ftwdt011->wdd, 0, dev);
	...
	/* Setup external clock if available */
	clk = devm_clk_get(dev, "extclk");
	if (!IS_ERR(clk)) {
		ftwdt011->extclk = clk;
		clk_prepare_enable(ftwdt011->extclk);
	}

	/* deasserted wdt reset */
	ftwdt011->rstc = devm_reset_control_get(&pdev->dev, "rstn");
	if (!IS_ERR_OR_NULL(ftwdt011->rstc)) {
		reset_control_deassert(ftwdt011->rstc);
	}
	...
	ftwdt011_set_timeout(&ftwdt011->wdd, ftwdt011->wdd.timeout);
