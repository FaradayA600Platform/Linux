...
#define SDHCI_FTSDC021_VENDOR2		0x108
...
#define SDHCI_FTSDC021_VENDOR8		0x120

extern void ftsdc021_wait_dll_lock(void);
extern void ftsdc021_reset_dll(void);
#endif
...
void sdhci_dumpregs(struct sdhci_host *host)
{
	...
	SDHCI_DUMP("Vendor[0]: 0x%08x | Vendor[1]:0x%08x\n",
		   sdhci_readl(host, SDHCI_FTSDC021_VENDOR0),
		   sdhci_readl(host, SDHCI_FTSDC021_VENDOR1));
	SDHCI_DUMP("Vendor[2]: 0x%08x | Vendor[3]:0x%08x\n",
		   sdhci_readl(host, SDHCI_FTSDC021_VENDOR2),
		   sdhci_readl(host, SDHCI_FTSDC021_VENDOR3));
#endif
...
#ifdef CONFIG_MMC_FTSDC021
		/*
		 * For ftsdc021 v1.10 after. This is work around solution when
		 * HW can not reflect the voltage switch in time.
		 * sd_1V8_fail_en(offset 0x120 bit31) must clear before 1.8V
		 * signal is enabled.
		 */
		ven_def = sdhci_readl(host, SDHCI_FTSDC021_VENDOR8);
		ven_def &= 0x7fffffff;
		sdhci_writel(host, ven_def, SDHCI_FTSDC021_VENDOR8);
#endif        
...
static int __sdhci_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	if (ctrl & SDHCI_CTRL_EXEC_TUNING) {
		...
		int delay;
		int win_start, win_size, tmp_start, tmp_size;
		u32 ven_def;

		delay = (sdhci_readl(host, SDHCI_FTSDC021_VENDOR3) >> 16) & 0x1F;
		ven_def = sdhci_readl(host, SDHCI_FTSDC021_VENDOR4);
		pr_info("%s: Tuning complete(0x%x), delay %d", mmc_hostname(host->mmc), ven_def, delay);
		...
		if (1) {
			...
			for (i = 0; i < 64; i++) {
				...
				asm("mov %0, %0, ror #1" : "=r" (ven_def) : "0" (ven_def));
			}
			if (win_size > 0) {
				delay = win_start + win_size / 2;
				if (delay > 31)
					delay -= 32;
				ven_def = sdhci_readl(host, SDHCI_FTSDC021_VENDOR3);
				ven_def &= ~SDHCI_SD_DELAY_VAL_MASK;
				ven_def |= (delay << SDHCI_SD_DELAY_VAL_SHIFT);
				sdhci_writel(host, ven_def, SDHCI_FTSDC021_VENDOR3);
				ftsdc021_wait_dll_lock();
				pr_info("%s: Tuning delay result not correct. New delay=%d\n", mmc_hostname(host->mmc), delay);
			}
		}
		return 0;
	}
